...
$isPayment = preg_match('/(ชำระเงิน|จ่ายเงิน|สแกนจ่าย|student pay|เติมเงิน|ซื้อข้าว|โรงอาหาร|กินข้าว|บัตรแทนเงินสด|student care)/ui', $userMessageRaw);
...
if ($resProfile) {
    while ($row = $resProfile->fetch_assoc()) {
        ...
        // --- ส่วนที่ปรับปรุง: เพิ่มเงื่อนไข topup เข้าไปในเงื่อนไขหลัก ---
        if ($keyMatch || 
            $cat === 'identity' || 
            $cat === 'motto' || 
            ...
            ($isPayment && ($cat === 'info' || $cat === 'topup')) // ✨ ดึงข้อมูลถ้าถามเรื่องจ่ายเงิน/เติมเงิน
        ) { 
            $context['info'] .= "- {$key}: {$val}\n";
            
            // เก็บ URL แผนที่หรือรูปภาพ ถ้ามีคำว่า http ในข้อมูล
            if (($cat === 'map' || $cat === 'uniform' || $cat === 'topup') && strpos($val, 'http') !== false) {
                $context['map_url'] = $val;
            }
        }
    }

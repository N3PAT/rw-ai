...
function send_json(array $data): void {
    global $conn;
    // ✅ แก้ไขปัญหาที่ 1: ปิด DB Connection อัตโนมัติทุกครั้งที่มีการส่งข้อมูลออกไป เพื่อคืน Resource ให้ Server
...
// ฟังก์ชันดึง IP Address (รองรับกรณีใช้ Proxy หรือ Cloudflare)
function getUserIP() {
    if (!empty($_SERVER['HTTP_CLIENT_IP'])) {
        return $_SERVER['HTTP_CLIENT_IP'];
    } elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
        // บางครั้ง Proxy ส่งมาหลาย IP ให้เอาตัวแรก
...
$userIP = getUserIP(); // ดึง IP Address ของผู้ใช้
...
// เตรียมตัวแปร Keyword เช็คไว้ล่วงหน้า (ย้ายออกจาก Loop เพื่อลดภาระการประมวลผล)
...
// ✅ แก้ไขปัญหาที่ 2: ปรับ Regex ให้ครอบคลุมคำว่า เสื้อพละ คณะสี รด. นศท. ปักดาว อย่างครบถ้วน
...
if ($resProfile) {
    while ($row = $resProfile->fetch_assoc()) {
        ...
        if ($row['info_key'] === 'แผนผัง') {
            // ✅ แก้ไขปัญหาที่ 4: ไม่ใช้ htmlspecialchars() ครอบตัวแปรเพื่อให้แสดงผลเป็น <img> ได้
            ...
            if (
                ...
                // ✅ ข้อมูลที่ดึงไปใช้ไม่มีการครอบ htmlspecialchars()
...
1. แยกแยะประเภทคำถาม:
   - **ถ้าเป็นคำถามทั่วไป/คุยเล่น:** (เช่น หวัดดี, กินข้าวยัง, ร้องเพลง) ให้คุยเล่นตอบโต้แบบรุ่นพี่ได้เลย
...
3. คะแนนพฤติกรรม: ย้ำบางครั้งที่ถามเรื่องพฤติกรรม​หรือคะแนนความ​ประพฤติ​หรือการต่อโควตาว่าโควตา ม.4 ต้องมีคะแนนพฤติกรรมไม่ต่ำกว่า 60 คะแนน
4. การแสดงรูปภาพ: 
...
if ($success && !empty($aiResponse)) {
    // 7.1 ลบข้อมูลที่เก่ากว่า 20 วันอัตโนมัติ (Cleanup)
    $conn->query("DELETE FROM chat_logs WHERE created_at < NOW() - INTERVAL 20 DAY");

    // 7.2 บันทึกข้อมูลการแชทครั้งนี้ลงฐานข้อมูล (Logging)
    ...
    // ✅ แก้ไขปัญหาที่ 3: สังเกตว่าในคำสั่ง send_json มีแค่ trim() ไม่มีพ่วง . "ครับ" มั่นใจได้ว่าบอทไม่เบิ้ลครับแน่นอน 
    // หมายเหตุ: ฟังก์ชัน send_json ถูกแก้ให้ทำหน้าที่ปิด $conn->close() อัตโนมัติแล้ว จึงไม่ต้องกังวลเรื่องการคืน Resource ในส่วนนี้

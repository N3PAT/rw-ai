...
// สลับลำดับ API Key เพื่อกระจาย Load
...
function checkRateLimit() {
    ...
    if (count($_SESSION['request_ts']) >= $limit) {
        send_json(["response" => "น้องใจเย็นๆ นะครับ พี่ขอเวลาพักจิบน้ำ 30 วินาที แล้วค่อยถามใหม่นะ ครับ"]);
    }
...
$keywords = [
    ...
    'rules'      => '/(กฎ|ระเบียบ|ผมยาว|คะแนน|หัก|ทัณฑ์บน|ตัดคะแนน)/u',
    'history'    => '/(ประวัติ|ก่อตั้ง|ปีที่ตั้ง|ผอ.คนแรก|เรื่องราว)/u',
    'curriculum' => '/(แผน|สายการเรียน|ห้องเรียน|ม.ต้น|ม.ปลาย|กิ๊ฟ|วิทย์|ศิลป์)/u'
];
...
// 🔥 ใช้โมเดลที่เสถียรสำหรับ API นี้
$cleanModel = "gemini-3.1-flash-lite-preview"; 
...
foreach ($config['gemini']['api_keys'] as $apiKey) {
    ...
    // ถ้าเจอ 429 (เกินโควตา) หรือ 500-599 (Server เอ๋อ/High Demand) ให้ลอง Key ถัดไป
...
// --- 6. LOGGING & CLEANUP ---
if ($success_ai && !empty($aiResponse)) {
    if (rand(1, 20) === 1) {
    ...
    $errorDetail = json_decode((string)$rawResponse, true);
    $errorMessage = $errorDetail['error']['message'] ?? 'ไม่ทราบสาเหตุแน่ชัด';
    
    // ปรับแต่งข้อความ Error 503 ให้เป็นมิตร "ออกสื่อได้"
    if ($httpCode === 503) {
        send_json(["response" => "ขออภัยครับน้องๆ ตอนนี้คนกำลังคุยกับพี่ RW-AI เยอะมากจนเซิร์ฟเวอร์รับไม่ไหว รบกวนน้องรอสักครู่แล้วลองถามใหม่อีกครั้งนะครับ ครับผม!"]);
    } else {

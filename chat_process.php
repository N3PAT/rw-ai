...
// --- 5. AI API CALL ---
// --- 5. AI API CALL (VERSION: ULTRA STABLE) ---
function callGeminiWithRetry($apiUrl, $payload) {
    $max_retries = 3; // เพิ่มเป็น 3 รอบ
    $retry_count = 0;
    ...
    while ($retry_count < $max_retries) {
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, 25); // ให้เวลามันคิดหน่อย 25 วิ
        ...
        if ($httpCode === 200 && $rawResponse !== false) {
            ...
            if (!empty($text)) return ['success' => true, 'text' => $text, 'code' => 200];
        }

        // ถ้าเจอ 503 หรือ 429 (Overloaded) ให้รอแล้วลองใหม่
        if ($httpCode === 503 || $httpCode === 429 || $httpCode === 0) {
            $retry_count++;
            if ($retry_count < $max_retries) {
                // รอบแรกรอ 1 วิ, รอบสองรอ 2 วิ (Exponential Backoff)
                sleep($retry_count * 1); 
                continue;
            }
        } else {
            // ถ้า Error อื่นๆ (เช่น 400 API Key พัง) ไม่ต้อง Retry ให้เสียเวลา
            break; 
        }
        $retry_count++;
    }
    return ['success' => false, 'text' => "", 'code' => $httpCode, 'error' => $curlError];
}

// ประมวลผลผลลัพธ์
$result = callGeminiWithRetry($apiUrl, $payload);
$aiResponse = $result['text'];
$success = $result['success'];
$httpCode = $result['code'];

// --- 6. LOGGING & CLEANUP (เหมือนเดิม) ---
if ($success && !empty($aiResponse)) {
    // ... โค้ดส่วนบันทึก Log และ Analytics ของนัท (คงเดิมไว้ได้เลย) ...
    ...
    send_json(["response" => trim($aiResponse), "log_id" => $lastId]);

} else {
    // ส่งข้อความ Error ที่อ่านง่ายขึ้น
    $errorMsg = "พี่ RW-AI ขออภัยครับ ระบบประมวลผลขัดข้องชั่วคราว (Code: $httpCode)";
    if ($httpCode === 503) $errorMsg = "ตอนนี้คนใช้บริการเยอะมาก พี่ประมวลผลไม่ทัน ลองใหม่อีกครั้งใน 10 วินาทีนี้นะครับ";
    if ($httpCode === 429) $errorMsg = "น้องถามเร็วไปนิด พี่ตอบไม่ทันแล้ว! รอสักครู่แล้วค่อยถามใหม่นะ";
    
    send_json(["response" => $errorMsg]);
}

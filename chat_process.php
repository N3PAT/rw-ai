...
// --- 5. AI API CALL (Multi-Model Support - Fixed 404) ---
$success = false;
$aiResponse = "";
$httpCode = 0;
...
foreach ($config['gemini']['models'] as $index => $currentModel) {
    ...
    // ลบช่องว่างที่อาจติดมาจากการก๊อปปี้ใน .env
    $currentModel = trim($currentModel);

    // ตรวจสอบโครงสร้างชื่อรุ่น (ต้องเริ่มด้วย models/ เสมอสำหรับ URL ของ Gemini)
    $modelPath = (strpos($currentModel, 'models/') === 0) ? $currentModel : "models/" . $currentModel;
    
    // สร้าง URL ที่ถูกต้อง
    $apiUrl = "https://generativelanguage.googleapis.com/v1beta/{$modelPath}:generateContent?key=" . urlencode((string)$config['gemini']['api_key']);

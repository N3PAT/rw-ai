...
// --- 5. AI API CALL (Multi-Model Support) ---
$success = false;
$aiResponse = "";
$httpCode = 0;
...
foreach ($config['gemini']['models'] as $index => $currentModel) {
    ...
    // สำคัญมาก: ลด Timeout เหลือ 12 วินาทีต่อ Model เพื่อให้จบงานก่อน Render ตัด (30s)
    ...
    if ($httpCode === 200 && $rawResponse !== false) {
        ...
        if (isset($resData['candidates'][0]['content']['parts'][0]['text'])) {
            ...
            $success = true;
            break; // ถ้าสำเร็จแล้วให้ออกจาก foreach ทันที
        }
    }

    // ถ้ายังไม่สำเร็จและยังมี Model ถัดไป ให้รอ 0.5 วินาทีก่อนลองใหม่

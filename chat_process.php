...
// เพิ่มคำว่า ม.ต้น, ม.ปลาย, มัธยมต้น, มัธยมปลาย ลงไปใน Regex
...
// 3.13 ข้อมูลแอปพลิเคชัน (Student Care)
if ($isApp) {
    ...
    // ดึงระเบียบที่เกี่ยวข้องกับการใช้ระบบ (ถ้ามี)
...
// 3.15 ข้อมูลแผนการเรียนและห้องเรียนประจำสายชั้น (NEW!)
if ($isCurriculum || $isRooms) {
    // ค้นหาแบบกว้างหรือค้นหาตามระดับชั้นที่ระบุในคำถาม
    $levelFilter = "";
...
5. การแสดงรูปภาพ (สำคัญมาก): 
   ...
   - หากมีรูปภาพมากกว่า 1 รูป ให้แสดงเรียงต่อกันลงมา6. ลักษณะการตอบ: สุภาพ ใจดี เป็นกันเอง และลงท้ายด้วย 'ครับ' ทุกประโยค
...
// 1. รับประวัติการแชทจากหน้าเว็บ (ถ้ามี)
$chatHistory = $input['history'] ?? [];
$contents = [];

// 2. ถ้ามีประวัติ ให้เอามาใส่ใน contents ก่อน
if (!empty($chatHistory) && is_array($chatHistory)) {
    // เอาประวัติมาใส่ แต่ตัดข้อความสุดท้ายของ user ออกก่อน (เดี๋ยวเราค่อยใส่แบบเต็มๆ)
    // เพราะเราจะเอาคำถามล่าสุดไปรวมกับ ข้อมูล Context ในขั้นตอนถัดไป
    $historyCount = count($chatHistory);
    for ($i = 0; $i < $historyCount - 1; $i++) {
        if (isset($chatHistory[$i]['role']) && isset($chatHistory[$i]['parts'][0]['text'])) {
             // เพิ่มเงื่อนไขเพื่อความชัวร์ว่าโครงสร้างถูกต้อง
             $contents[] = [
                 "role" => $chatHistory[$i]['role'], // จะเป็น 'user' หรือ 'model'
...
// 3. เอาคำถามล่าสุดของ User มารวมกับ $prompt (ข้อมูลจากฐานข้อมูล) เป็นอันสุดท้าย
$contents[] = [
    "role" => "user", 
    "parts" => [["text" => $prompt]]
];
...
$jsonPayload = json_encode([
    "contents" => $contents, // ใช้ $contents แบบใหม่ที่มีความจำแล้ว
    "generationConfig" => [
        "temperature" => 0.2,
        "topP" => 0.95,
        "topK" => 64,
